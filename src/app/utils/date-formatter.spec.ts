...
describe('DateFormatter Utility Tests', () => {
  ...
  it('should generate MONTH grain headers sequentially backward', () => {
    ...
    expect(result[0].colId).toBe('C7_3');
    expect(result[0].label).toBe('March 2026');
    expect(result[0].periodOffset).toBe(-3);
    ...
    expect(result[2].colId).toBe('C7_1');
    expect(result[2].label).toBe('May 2026');
    expect(result[2].periodOffset).toBe(-1);
  });
  ...
  it('should generate WEEK grain headers with ISO week-of-year labels', () => {
    ...
    expect(result[0].colId).toBe('C7_2');
    expect(result[0].label).toBe('WK21');
    expect(result[0].periodOffset).toBe(-2);

    // offset -1: Monday is 2026-05-25 → ISO Thursday anchor 2026-05-28 → ISO WK22
    expect(result[1].colId).toBe('C7_1');
    expect(result[1].label).toBe('WK22');
    expect(result[1].periodOffset).toBe(-1);
  });
  ...
  it('should generate DAY grain headers with short date stamps', () => {
    ...
    expect(result[0].colId).toBe('C7_3');
    expect(result[0].label).toBe('01 Jun');
    expect(result[0].periodOffset).toBe(-3);
    ...
    expect(result[2].colId).toBe('C7_1');
    expect(result[2].label).toBe('03 Jun');
    expect(result[2].periodOffset).toBe(-1);
  });
  ...
  it('should generate QUARTER grain headers with Qn YYYY labels', () => {
    ...
    expect(result[0].colId).toBe('C7_2');
    expect(result[0].label).toBe('Q4 2025');
    expect(result[0].periodOffset).toBe(-2);

    // offset -1 quarter: March 2026 (Q1)
    expect(result[1].colId).toBe('C7_1');
    expect(result[1].label).toBe('Q1 2026');
    expect(result[1].periodOffset).toBe(-1);
  });
});
